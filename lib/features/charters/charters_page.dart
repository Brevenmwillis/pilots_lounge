...
class _ChartersPageState extends State<ChartersPage> {
  ...
  void _showCharterBookingDialog(Aircraft aircraft) {
    showDialog(
      context: context,
      barrierDismissible: true,
      builder: (context) => AlertDialog(
        title: Text('Book Charter - ${aircraft.make} ${aircraft.model}'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Text('Rate: \$${aircraft.price}/hour'),
            const SizedBox(height: 16),
            const Text('Charter booking functionality coming soon!'),
            const SizedBox(height: 8),
            const Text('Contact the operator directly for booking.'),
          ],
        ),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('Charter booking request sent!')),
              );
            },
                        ...
                        children: [
                          ...
                          Expanded(
                            child: ElevatedButton(
                              onPressed: () {
                                Navigator.of(context).pop();
                                _showCharterBookingDialog(aircraft);
                              },
                              style: ElevatedButton.styleFrom(
                                padding: const EdgeInsets.symmetric(vertical: 12),
                              ),
                  ...
                  Expanded(
                    child: ListView.builder(
                      ...
                      itemBuilder: (_, i) => Padding(
                        ...
                        child: CharterCard(
                          aircraft: _charters[i],
                          onTap: () => _showCharterDetails(_charters[i]),
                        ),
                      ),
                    ),
                  ),
...
class CharterCard extends StatelessWidget {
  ...
  const CharterCard({required this.aircraft, required this.onTap, super.key});
